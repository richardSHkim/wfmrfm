...
def main() -> None:
    ...
    for ep, demo in enumerate(demos):
        ...
        total += n
        print(f"[conv] {demo} -> episode_{ep:06d}: {n} frames  eef|state|act ok", flush=True)
    f.close()
    ...
    info = {
        ...
        "total_episodes": len(demos), "total_frames": total, "total_tasks": 1,
        "total_videos": len(demos), "total_chunks": 1, "chunks_size": 1000, "fps": FPS,
    ...
    print(f"[conv] DONE: {len(demos)} episodes, {total} frames -> {out}")
